...
describe('The optimizer', () => {
  // ── Constant folding ─────────────────────────────────────────────────────────
  it('folds 3 + 4 to 7', () =>
    assert.equal(firstInit('let x as number be 3 + 4'), 7))
  it('folds 10 - 3 to 7', () =>
    assert.equal(firstInit('let x as number be 10 - 3'), 7))
  it('folds 3 * 4 to 12', () =>
    assert.equal(firstInit('let x as number be 3 * 4'), 12))
  it('folds 10 / 2 to 5', () =>
    assert.equal(firstInit('let x as number be 10 / 2'), 5))
  it('folds 10 % 3 to 1', () =>
    assert.equal(firstInit('let x as number be 10 % 3'), 1))
  it('folds -10 to -10', () =>
    assert.equal(firstInit('let x as number be -10'), -10))
  it('folds not true to false', () =>
    assert.equal(firstInit('let x as boolean be not true'), false))
  it('folds not false to true', () =>
    assert.equal(firstInit('let x as boolean be not false'), true))
  it('folds 3 is 3 to true', () =>
    assert.equal(firstInit('let x as boolean be 3 is 3'), true))
  it('folds 3 is 4 to false', () =>
    assert.equal(firstInit('let x as boolean be 3 is 4'), false))
  it('folds 3 is not 4 to true', () =>
    assert.equal(firstInit('let x as boolean be 3 is not 4'), true))
  it('folds 5 is greater than 3 to true', () =>
    assert.equal(firstInit('let x as boolean be 5 is greater than 3'), true))
  ...
  it('simplifies x + 0 to x', () => {
    assert.equal(
      optimized('let x as number be 5\nlet y as number be x + 0').statements[1]
        .initializer.kind,
      'Variable'
    )
  })
  it('simplifies 0 + x to x', () => {
    assert.equal(
      optimized('let x as number be 5\nlet y as number be 0 + x').statements[1]
        .initializer.kind,
      'Variable'
    )
  })
  ...
  it('simplifies x * 1 to x', () => {
    assert.equal(
      optimized('let x as number be 5\nlet y as number be x * 1').statements[1]
        .initializer.kind,
      'Variable'
    )
  })
  it('simplifies 1 * x to x', () => {
    assert.equal(
      optimized('let x as number be 5\nlet y as number be 1 * x').statements[1]
        .initializer.kind,
      'Variable'
    )
  })
  ...
  it('eliminates while-false entirely', () => {
    assert.equal(
      stmtCount('let x as number be 1\nas long as false { display x }'),
      1
    )
  })
  ...
  it('folds x is less than or equal to y (<= branch)', () => {
    assert.equal(
      firstInit('let x as boolean be 3 is less than or equal to 3'),
      true
    )
  })
  it('folds x is greater than or equal to y (>= branch)', () => {
    assert.equal(
      firstInit('let x as boolean be 5 is greater than or equal to 3'),
      true
    )
  })
