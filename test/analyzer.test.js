...
const semanticChecks = [
  ...
  [
    ...
    'define function: label() outputs string { output "ok" }',
  ],
  [
    'function returns boolean',
    'define function: yes() outputs boolean { output true }',
  ],
  [
    'function two params',
  ...
  [
    ...
    'define function: f() outputs void { display "x" }\nf()',
  ],
  [
  ...
  [
    'function call with matching types',
    'define function: f(s as string) outputs void { display s }\nf("hello")',
  ],
  [
    'recursive function',
    'define function: f(n as number) outputs number { output f(n) }',
  ],
  ...
  [
    'short if with boolean var',
    'let flag as boolean be true\nif flag { display "yes" }',
  ],
  [
    'if otherwise with boolean',
    'let flag as boolean be false\nif flag { display "yes" } otherwise { display "no" }',
  ],
  [
  ...
  // ForEach
  [
    'foreach over list containing number',
    'let nums as list containing number be [1,2]\ngo through each n in nums { display n }',
  ],
  [
    'foreach over list containing string',
    'let words as list containing string be ["a","b"]\ngo through each w in words { display w }',
  ],
  [
    'foreach with stop',
    'let nums as list containing number be [1]\ngo through each n in nums { stop }',
  ],

  // Scoping
  [
    'outer var in if body',
    'let x as number be 5\nlet flag as boolean be true\nif flag { display x }',
  ],
  [
    'outer var in while body',
    'let x as number be 5\nlet go as boolean be true\nas long as go { display x }',
  ],
  [
    'stop in if inside loop',
...
const semanticErrors = [
  ...
  [
    'return string from number fn',
    'define function: f() outputs number { output "oops" }',
    /Cannot assign/,
  ],
  [
    'return number from string fn',
    'define function: f() outputs string { output 42 }',
    /Cannot assign/,
  ],
  [
    'return number from boolean fn',
    'define function: f() outputs boolean { output 1 }',
    /Cannot assign/,
  ],

  // Argument type mismatch
  [
    'string arg to number param',
    'define function: f(x as number) outputs void { display x }\nf("bad")',
    /Cannot assign/,
  ],
  [
    'number arg to string param',
    'define function: f(s as string) outputs void { display s }\nf(42)',
    /Cannot assign/,
  ],
  [
    'boolean arg to number param',
    'define function: f(x as number) outputs void { display x }\nf(true)',
    /Cannot assign/,
  ],
  ...
  [
    'or on number and boolean',
    'let x as number be 1\nlet y as number be 2\nlet z as boolean be x or y',
    /Expected boolean/,
  ],
  [
    'not on number',
  ...
  [
    'subtract strings',
    'let s as string be "a"\nlet t as string be "b"\nlet x as number be s - t',
    /Expected number/,
  ],
  [
    'compare number and string',
    'let x as number be 5\nlet s as string be "hi"\nlet y as boolean be x is s',
    /Cannot compare/,
  ],
  ...
  [
    'foreach on number',
    'let x as number be 5\ngo through each n in x { display n }',
    /Expected a list/,
  ],
  [
    'foreach on string',
    'let s as string be "hi"\ngo through each c in s { display c }',
    /Expected a list/,
  ],
  [
    'mixed map value types',
    'let m as map linking string to number be {"a" -> 1, "b" -> "oops"}',
    /All map values must have the same type/,
  ],
  [
    'assign wrong map value type',
    'let m as map linking string to number be {"a" -> "hello"}',
    /Cannot assign/,
  ],
  [
    'assign map to list variable',
    'let m as list containing number be {"a" -> 1}',
    /Cannot assign/,
  ],
  [
    'field not found on group',
    'group Point: x as number\ndisplay Point.y',
    /has no field/,
  ],
  ...
  [
    'redeclared function',
    'define function: f() outputs void { display "a" }\ndefine function: f() outputs void { display "b" }',
    /already declared/,
  ],
  [
    'duplicate param names',
    'define function: f(x as number, x as number) outputs void { display x }',
    /already declared/,
  ],
  ...
  [
    'stop in function not in loop',
    'define function: f() outputs void { stop }',
    /'stop' can only appear inside a loop/,
  ],
  [
    'output outside function',
    'output 5',
    /'output' can only appear inside a function/,
  ],

  // Duplicate group fields
  [
    'duplicate group fields',
    'group Bad: x as number, x as string',
    /duplicate fields/,
  ],
]
...
describe('The analyzer', () => {
  ...
  it('call node carries return type', () => {
    const prog = check(
      'define function: f(x as number) outputs string { output "ok" }\nlet s as string be f(1)'
    )
  ...
  it('map expression carries map type with correct key and value types', () => {
    const prog = check(
      'let m as map linking string to number be {"score" -> 99}'
    )
